...
\subsection{Estabilizar la red (DoubleNetworkAgent)}

Lograda la resolución del problema, pero aun con la gran diferencia de los resultados entre diferentes simulaciones, y siguiendo las conclusiones sacadas por el departamento de DeepMind de Google en distintos artículos publicados (poner referencias), se explica que la mejor forma para lograr la estabilización de la red es la utilización de 2 redes diferentes e idénticas, de modo que la primera red (agente) se utilice para guardar los datos previos con los cuales tomar las decisiones en cuanto a los movimientos a ejecutar, y una segunda red (aprendizaje) en la cual ir guardando los valores de la ejecución actualizados, de modo que cada X pasos ambas redes son comparadas, guardando la mejor de ellas como agente para la toma de decisiones, y utilizlando la otra para el aprendizaje.
